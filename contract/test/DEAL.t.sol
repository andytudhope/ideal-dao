...
contract DEALTest is Test {
    ...
    function _testMintWithToken(address investor, ERC20 token, uint256 mintAmount) internal {
        ...
        assertEq(
            token.balanceOf(address(deal)) - beforeReserveBal,
            mintAmount
        );
        assertApproxEqAbs(
    ...
    function _testPermitAndMint(ERC20 token, uint256 amount) internal {
        ...
        bytes32 permitHash = _getEIP2612PermitHash(
            address(token),
            holder,
            address(deal),
            amount,
    ...
    function testBurnUSDS() public {
        ...
        assertApproxEqAbs(
            usds.balanceOf(hackerman) - beforeUsdsBal,
            predictedReturn,
            ACCURACY
        );
        assertApproxEqAbs(
            beforeReserveBal - usds.balanceOf(address(deal)),
            predictedReturn,
            ACCURACY
        );
        assertEq(
