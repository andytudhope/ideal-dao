...
contract ProposalsTest is Test {
    ...
    function testCreateProposal() public {
        ...
        emit ProposalCreated(0, proposer, PROPOSAL_AMOUNT, beneficiary, "test");

        uint256 proposalId = proposals.createProposal(PROPOSAL_AMOUNT, beneficiary, "test");

        (uint256 dealRequired,, address payableAddress, string memory documentURI) = proposals.getProposal(proposalId);
    ...
    function testFundProposal() public {
        ...
        uint256 proposalId = proposals.createProposal(
            PROPOSAL_AMOUNT, beneficiary, "ipfs://QmZ4tDuvesekSs4qM5ZBKpXiZGun7S2CYtEZRB3DYXkjGx"
        );
        ...
        emit ProposalFunded(proposalId, investor, PROPOSAL_AMOUNT, initialBeneficiaryBalance + PROPOSAL_AMOUNT);

        proposals.fundProposal(proposalId, PROPOSAL_AMOUNT);

        // Verify balances after funding
        assertEq(dealToken.balances(investor), initialInvestorBalance - PROPOSAL_AMOUNT);
        assertEq(dealToken.balances(beneficiary), initialBeneficiaryBalance + PROPOSAL_AMOUNT);
    }
    ...
    function testMultipleProposalsAndFunding() public {
        ...
        uint256 proposalId2 = proposals.createProposal(
            PROPOSAL_AMOUNT * 2, address(6), "ipfs://QmYwAPJzv5CZsnA625s3Xf2nemtYgPpHdWEz79ojWnPbdG"
        );
